...
// start convert_from_v8 block
  ...
  if (info[0].IsBuffer()) {
    // Buffer path: copy raw bytes directly — preserves binary content byte-for-byte
    Napi::Buffer<char> buf = info[0].As<Napi::Buffer<char>>();
    from_data = (const void *) malloc(buf.Length() + 1);
    memcpy((void *)from_data, buf.Data(), buf.Length());
    memset((void *)(((char *)from_data) + buf.Length()), 0, 1);
  } else {
    // String path: UTF-8 conversion (original behavior, fine for text content)
