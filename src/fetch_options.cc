...
nodegit::ConfigurableClassWrapper<GitFetchOptionsTraits>::v8ConversionResult ConfigurableGitFetchOptions::fromJavascript(nodegit::Context *nodegitContext, Napi::Value input) {
  if (!input.IsObject()) {
    return {
  ...
  // unpack the data into the correct fields
         {
          ...
          if (!maybeNestedObject.IsUndefined() && !maybeNestedObject.IsNull()) {
           ...
           {
          Napi::Value maybeNumber = nodegit::safeGetField(inputObj, "prune");
          if (!maybeNumber.IsUndefined() && !maybeNumber.IsNull()) {
            if (!maybeNumber.IsNumber()) {
              return {
            ...
            output->raw->prune = static_cast<git_fetch_prune_t>(maybeNumber.As<Napi::Number>().Int32Value());
          }
        }
          {
          ...
          if (!maybeNestedObject.IsUndefined() && !maybeNestedObject.IsNull()) {
           ...
           {
          Napi::Value maybeNumber = nodegit::safeGetField(inputObj, "updateFetchhead");
          if (!maybeNumber.IsUndefined() && !maybeNumber.IsNull()) {
            if (!maybeNumber.IsNumber()) {
              return {
            ...
            output->raw->update_fetchhead = static_cast<int>(maybeNumber.As<Napi::Number>().Int32Value());
          }
        }
           {
          Napi::Value maybeNumber = nodegit::safeGetField(inputObj, "downloadTags");
          if (!maybeNumber.IsUndefined() && !maybeNumber.IsNull()) {
            if (!maybeNumber.IsNumber()) {
              return {
            ...
            output->raw->download_tags = static_cast<git_remote_autotag_option_t>(maybeNumber.As<Napi::Number>().Int32Value());
          }
        }
