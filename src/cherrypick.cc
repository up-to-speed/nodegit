...
using namespace std;
  ...
  void GitCherrypick::InitializeComponent(Napi::Object target, nodegit::Context *nodegitContext) {
    Napi::Env env = target.Env();
    Napi::HandleScope scope(env);
    Napi::External<void> nodegitExternal = Napi::External<void>::New(env, nodegitContext);

      Napi::Function object = Napi::Function::New(env, Cherrypick);
         object.Set("cherrypick", Napi::Function::New(env, Cherrypick));
         object.Set("commit", Napi::Function::New(env, Commit));
  
    target.Set(
      Napi::String::New(env, "Cherrypick"),
      object
    );
  }
 ...
 * @param Repository repo
   ...
   * @param CherrypickOptions cherrypick_options
     */
Napi::Value GitCherrypick::Cherrypick(const Napi::CallbackInfo& info) {
...
// end convert_from_v8 block
          ...
          if (info[2].IsNull() || info[2].IsUndefined()) {
            baton->cherrypick_options = nullptr;
          } else
            {
            auto conversionResult = ConfigurableGitCherrypickOptions::fromJavascript(nodegitContext, info[2]);
            ...
            cleanupHandles["cherrypick_options"] = convertedObject;
            baton->cherrypick_options = convertedObject->GetValue();
          }
 
  Napi::FunctionReference callback = Napi::Persistent(info[info.Length() - 1].As<Napi::Function>());
  CherrypickWorker *worker = new CherrypickWorker(baton, std::move(callback), cleanupHandles);

             worker->Reference<GitRepository>("repo", info[0]);
               worker->Reference<GitCommit>("commit", info[1]);
             worker->Reference("cherrypick_options", info[2]);
 
  nodegitContext->QueueWorker(worker);
  return env.Undefined();
}

nodegit::LockMaster GitCherrypick::CherrypickWorker::AcquireLocks() {
...
void GitCherrypick::CherrypickWorker::HandleErrorCallback() {
  if (!GetIsCancelled()) {
    Napi::Env env = Env();
    Napi::Object err = Napi::Error::New(env, ErrorMessage()).Value().As<Napi::Object>();
    err.Set(Napi::String::New(env, "errorFunction"), Napi::String::New(env, "Cherrypick.cherrypick"));
    napi_value argv[1] = {
      err
    };
  ...
  Napi::Env env = Env();
    ...
    if (baton->error) {
      ...
      if (baton->error->message) {
        ...
        err = Napi::Error::New(env, "Method cherrypick has thrown an error.").Value().As<Napi::Object>();
      }
      ...
      if (!callbackFired) {
        Napi::Object err = Napi::Error::New(env, "Method cherrypick has thrown an error.").Value().As<Napi::Object>();
        err.Set(Napi::String::New(env, "errno"), Napi::Number::New(env, baton->error_code));
        err.Set(Napi::String::New(env, "errorFunction"), Napi::String::New(env, "Cherrypick.cherrypick"));
        napi_value argv[1] = {
          err
...
/*
  * @param Repository repo
   ...
   */
Napi::Value GitCherrypick::Commit(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  ...
  if (info.Length() == 1 || !info[1].IsObject()) {
    Napi::Error::New(env, "Commit cherrypick_commit is required.").ThrowAsJavaScriptException();
    return env.Undefined();
  }
...
from_cherrypick_commit = Napi::ObjectWrap<GitCommit>::Unwrap(info[1].As<Napi::Object>())->GetValue();
// end convert_from_v8 block
...
// end convert_from_v8 block
  ...
  CommitWorker *worker = new CommitWorker(baton, std::move(callback), cleanupHandles);
             ...
             worker->Reference<GitRepository>("repo", info[0]);
               worker->Reference<GitCommit>("cherrypick_commit", info[1]);
...
nodegit::LockMaster GitCherrypick::CommitWorker::AcquireLocks() {
  nodegit::LockMaster lockMaster(
    /*asyncAction: */true
          ,baton->repo
...
void GitCherrypick::CommitWorker::HandleErrorCallback() {
  if (!GetIsCancelled()) {
    Napi::Env env = Env();
    Napi::Object err = Napi::Error::New(env, ErrorMessage()).Value().As<Napi::Object>();
    err.Set(Napi::String::New(env, "errorFunction"), Napi::String::New(env, "Cherrypick.commit"));
    napi_value argv[1] = {
      err
    };
    callback.Call(env.Undefined(), 1, argv);
