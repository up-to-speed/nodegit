...
using namespace std;
  ...
  void GitCheckout::InitializeComponent(Napi::Object target, nodegit::Context *nodegitContext) {
    Napi::Env env = target.Env();
    Napi::HandleScope scope(env);
    Napi::External<void> nodegitExternal = Napi::External<void>::New(env, nodegitContext);

       Napi::Object object = Napi::Object::New(env);
         object.Set("head", Napi::Function::New(env, Head));
         object.Set("index", Napi::Function::New(env, Index));
         object.Set("tree", Napi::Function::New(env, Tree));
  
    target.Set(
      Napi::String::New(env, "Checkout"),
      object
    );
  }
 ...
 * @param Repository repo
   * @param CheckoutOptions opts
     */
Napi::Value GitCheckout::Head(const Napi::CallbackInfo& info) {
...
// end convert_from_v8 block
          ...
          if (info[1].IsNull() || info[1].IsUndefined()) {
            ...
            {
            auto conversionResult = ConfigurableGitCheckoutOptions::fromJavascript(nodegitContext, info[1]);
...
nodegit::LockMaster GitCheckout::HeadWorker::AcquireLocks() {
...
void GitCheckout::HeadWorker::HandleErrorCallback() {
  if (!GetIsCancelled()) {
    Napi::Env env = Env();
    Napi::Object err = Napi::Error::New(env, ErrorMessage()).Value().As<Napi::Object>();
    err.Set(Napi::String::New(env, "errorFunction"), Napi::String::New(env, "Checkout.head"));
    napi_value argv[1] = {
      err
    };
 ...
 * @param Repository repo
   ...
   * @param CheckoutOptions opts
     */
Napi::Value GitCheckout::Index(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
...
// end convert_from_v8 block
          ...
          if (info[2].IsNull() || info[2].IsUndefined()) {
            ...
            {
            auto conversionResult = ConfigurableGitCheckoutOptions::fromJavascript(nodegitContext, info[2]);
            if (!conversionResult.result) {
              Napi::Error::New(env, conversionResult.error).ThrowAsJavaScriptException();
              return env.Undefined();
            }
...
nodegit::LockMaster GitCheckout::IndexWorker::AcquireLocks() {
  nodegit::LockMaster lockMaster(
    /*asyncAction: */true
          ,baton->repo
          ,baton->index
...
void GitCheckout::IndexWorker::HandleErrorCallback() {
  if (!GetIsCancelled()) {
    Napi::Env env = Env();
    Napi::Object err = Napi::Error::New(env, ErrorMessage()).Value().As<Napi::Object>();
    err.Set(Napi::String::New(env, "errorFunction"), Napi::String::New(env, "Checkout.index"));
    napi_value argv[1] = {
      err
    };
    callback.Call(env.Undefined(), 1, argv);
 ...
 * @param Repository repo
   ...
   * @param CheckoutOptions opts
     */
Napi::Value GitCheckout::Tree(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
...
// start convert_from_v8 block
  const git_object * from_treeish = NULL;
    if ((info.Length() - 1) > 1 && info[1].IsObject()) {
from_treeish = Napi::ObjectWrap<GitObject>::Unwrap(info[1].As<Napi::Object>())->GetValue();
  }
  else {
    from_treeish = 0;
  }
// end convert_from_v8 block
  ...
  Napi::FunctionReference callback = Napi::Persistent(info[info.Length() - 1].As<Napi::Function>());
  TreeWorker *worker = new TreeWorker(baton, std::move(callback), cleanupHandles);

             worker->Reference<GitRepository>("repo", info[0]);
               worker->Reference<GitObject>("treeish", info[1]);
             worker->Reference("opts", info[2]);
...
nodegit::LockMaster GitCheckout::TreeWorker::AcquireLocks() {
  nodegit::LockMaster lockMaster(
    /*asyncAction: */true
          ,baton->repo
          ,baton->treeish
...
void GitCheckout::TreeWorker::HandleErrorCallback() {
  if (!GetIsCancelled()) {
    Napi::Env env = Env();
    Napi::Object err = Napi::Error::New(env, ErrorMessage()).Value().As<Napi::Object>();
    err.Set(Napi::String::New(env, "errorFunction"), Napi::String::New(env, "Checkout.tree"));
    napi_value argv[1] = {
      err
    };
    callback.Call(env.Undefined(), 1, argv);
  ...
  Napi::Env env = Env();
    ...
    if (baton->error) {
      ...
      if (baton->error->message) {
        ...
        err = Napi::Error::New(env, "Method tree has thrown an error.").Value().As<Napi::Object>();
      }
      err.Set(Napi::String::New(env, "errno"), Napi::Number::New(env, baton->error_code));
      err.Set(Napi::String::New(env, "errorFunction"), Napi::String::New(env, "Checkout.tree"));
      napi_value argv[1] = {
      ...
      if (!callbackFired) {
        Napi::Object err = Napi::Error::New(env, "Method tree has thrown an error.").Value().As<Napi::Object>();
        err.Set(Napi::String::New(env, "errno"), Napi::Number::New(env, baton->error_code));
        err.Set(Napi::String::New(env, "errorFunction"), Napi::String::New(env, "Checkout.tree"));
        napi_value argv[1] = {
          err
