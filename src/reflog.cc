...
using namespace std;
  ...
  void GitReflog::InitializeComponent(Napi::Object target, nodegit::Context *nodegitContext) {
    Napi::Env env = target.Env();
    Napi::HandleScope scope(env);

    Napi::External<void> nodegitExternal = Napi::External<void>::New(env, nodegitContext);

    Napi::Function constructor_template = DefineClass(env, "Reflog", {
          InstanceMethod("append", &GitReflog::Append, static_cast<napi_property_attributes>(napi_writable | napi_configurable)),
             StaticMethod("delete", &GitReflog::Delete, static_cast<napi_property_attributes>(napi_writable | napi_configurable)),
            InstanceMethod("drop", &GitReflog::Drop, static_cast<napi_property_attributes>(napi_writable | napi_configurable)),
            InstanceMethod("entryByIndex", &GitReflog::EntryByindex, static_cast<napi_property_attributes>(napi_writable | napi_configurable)),
            InstanceMethod("entrycount", &GitReflog::Entrycount, static_cast<napi_property_attributes>(napi_writable | napi_configurable)),
             StaticMethod("read", &GitReflog::Read, static_cast<napi_property_attributes>(napi_writable | napi_configurable)),
             StaticMethod("rename", &GitReflog::Rename, static_cast<napi_property_attributes>(napi_writable | napi_configurable)),
            InstanceMethod("write", &GitReflog::Write, static_cast<napi_property_attributes>(napi_writable | napi_configurable)),
          StaticMethod("getSelfFreeingInstanceCount", &GitReflog::GetSelfFreeingInstanceCount, static_cast<napi_property_attributes>(napi_writable | napi_configurable)),
      StaticMethod("getNonSelfFreeingConstructedCount", &GitReflog::GetNonSelfFreeingConstructedCount, static_cast<napi_property_attributes>(napi_writable | napi_configurable)),
    });

    InitializeTemplate(constructor_template);

    nodegitContext->SaveToPersistent("GitReflog::Template", constructor_template);
    target.Set(Napi::String::New(env, "Reflog"), constructor_template);
  }
...
Napi::Value GitReflog::Append(const Napi::CallbackInfo& info) {
...
// end convert_from_v8 block
  ...
  { // lock master scope start
    nodegit::LockMaster lockMaster(
      /*asyncAction: */false
            ,
              Napi::ObjectWrap<GitReflog>::Unwrap(info.This().As<Napi::Object>())->GetValue()
            ,
              from_id
            ,
              from_committer
 ...
 * @param Repository repo
   * @param String name
     * @return Number  result    */
Napi::Value GitReflog::Delete(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  Napi::EscapableHandleScope scope(env);
...
/*
   ...
   * @param Number rewrite_previous_entry
     * @return Number  result    */
Napi::Value GitReflog::Drop(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  Napi::EscapableHandleScope scope(env);
  ...
  if (info.Length() == 1 || !info[1].IsNumber()) {
    Napi::Error::New(env, "Number rewrite_previous_entry is required.").ThrowAsJavaScriptException();
    return env.Undefined();
  }
...
// start convert_from_v8 block
  int from_rewrite_previous_entry;
      from_rewrite_previous_entry = (int)   info[1].As<Napi::Number>().DoubleValue ();
// end convert_from_v8 block
...
Napi::Value GitReflog::EntryByindex(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  Napi::EscapableHandleScope scope(env);
...
// start convert_to_v8 block
    if (result != NULL) {
      Napi::Array owners = Napi::Array::New(env, 0);
          ...
          v8ConversionSlot = GitReflogEntry::New(
        env,
        result,
        false
          , owners
...
/*
     * @return Number  result    */
Napi::Value GitReflog::Entrycount(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  Napi::EscapableHandleScope scope(env);
...
/*
  * @param Repository repo
   ...
   */
Napi::Value GitReflog::Read(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
...
nodegit::LockMaster GitReflog::ReadWorker::AcquireLocks() {
  nodegit::LockMaster lockMaster(
    /*asyncAction: */true
          ,baton->repo
          ,baton->name
...
void GitReflog::ReadWorker::HandleErrorCallback() {
  if (!GetIsCancelled()) {
    Napi::Env env = Env();
    Napi::Object err = Napi::Error::New(env, ErrorMessage()).Value().As<Napi::Object>();
    err.Set(Napi::String::New(env, "errorFunction"), Napi::String::New(env, "Reflog.read"));
    napi_value argv[1] = {
      err
    };
    ...
    if (baton->out != NULL) {
      Napi::Array owners = Napi::Array::New(env, 0);
            owners.Set(Napi::Number::New(env, owners.Length()), this->GetFromPersistent("repo").As<Napi::Object>());
             v8ConversionSlot = GitReflog::New(
        env,
        baton->out,
        true
          , owners
 ...
 * @param Repository repo
   ...
   * @param String name
     * @return Number  result    */
Napi::Value GitReflog::Rename(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  Napi::EscapableHandleScope scope(env);
  ...
  if (info.Length() == 1 || !info[1].IsString()) {
    Napi::Error::New(env, "String old_name is required.").ThrowAsJavaScriptException();
    return env.Undefined();
  }
...
// start convert_from_v8 block
  ...
  // copy the characters from the nodejs string into our C-string (used instead of strdup or strcpy because nulls in
  // the middle of strings are valid coming from nodejs):
  memcpy((void *)from_old_name, old_name.c_str(), old_name.length());
  // ensure the final byte of our new string is null, extra casts added to ensure compatibility with various C types
  // used in the nodejs binding generation:
...
/*
     */
Napi::Value GitReflog::Write(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  ...
  std::map<std::string, std::shared_ptr<nodegit::CleanupHandle>> cleanupHandles;
        ...
        baton->reflog = Napi::ObjectWrap<GitReflog>::Unwrap(info.This().As<Napi::Object>())->GetValue();

  Napi::FunctionReference callback = Napi::Persistent(info[info.Length() - 1].As<Napi::Function>());
  WriteWorker *worker = new WriteWorker(baton, std::move(callback), cleanupHandles);

        worker->Reference<GitReflog>("reflog", info.This().As<Napi::Object>());

  nodegitContext->QueueWorker(worker);
  return env.Undefined();
}

nodegit::LockMaster GitReflog::WriteWorker::AcquireLocks() {
  nodegit::LockMaster lockMaster(
    /*asyncAction: */true
          ,baton->reflog
  );
...
void GitReflog::WriteWorker::HandleErrorCallback() {
  if (!GetIsCancelled()) {
    Napi::Env env = Env();
    Napi::Object err = Napi::Error::New(env, ErrorMessage()).Value().As<Napi::Object>();
    err.Set(Napi::String::New(env, "errorFunction"), Napi::String::New(env, "Reflog.write"));
    napi_value argv[1] = {
      err
    };
    callback.Call(env.Undefined(), 1, argv);
